...
//FILE WITH OPTIMIZED OBJECT COLLISION ALGORITHM
		...
		double s;
				...
				while(n1 != NULL){
					...
					while(n2 != NULL){
						...
						c = cos(theta);
						s = sin(theta);
						x1 = obj1.x_min + (obj1.distance * c);
						y1 = obj1.y_min + (obj1.distance * s);
						x2 = obj1.x_max + (obj1.distance * c);
						y2 = obj1.y_max + (obj1.distance * s);

						double obj1_xc = (x1+x2) * 0.5; //x-coordinate of center of object 1
						double obj1_yc = (y1+y2) * 0.5; //y-coordinate of center of object 1
						...
						c = cos(theta);
						s = sin(theta);
                                                x1 = obj2.x_min + (obj2.distance * c);
                                                y1 = obj2.y_min + (obj2.distance * s);
                                                x2 = obj2.x_max + (obj2.distance * c);
                                                y2 = obj2.y_max + (obj2.distance * s);

						double obj2_xc = (x1+x2) * 0.5; //x-coordinate of center of object 2
						double obj2_yc = (y1+y2) * 0.5; //y-coordinate of center of object 2
						double obj2_xw = x2 - x1;  //width of object 2
						double obj2_yh = y2 - y1;  //height of object 2

						double len1; //horizontal difference between the centers of the detection areas
						double len2; //vertical difference between the centers of the detection areas
						//find the difference between the x and y centers of the two objects
						if(obj1_xc > obj2_xc){
							len1 = obj1_xc - obj2_xc;
						}
						else{
							len1 = obj2_xc - obj1_xc;
						}
						if(obj1_yc > obj2_yc){
							len2 = obj1_yc - obj2_yc;
						}
						else{
							len2 = obj2_yc - obj1_yc;
						}
						//core of the algorithm(if both are true, a collision has occurred:
						if((len1 <= ((obj1_xw+obj2_xw) * 0.5)) && (len2 <= ((obj1_yh+obj2_yh) * 0.5))){
							...
							angle1 = n1->data.angle;
							n1->data.angle = n2->data.angle; //update object 1 to the new angle
	...
	void System::addObject(int size){
		...
		while(placed){
			...
			while(n != NULL && valid){
				...
				double obj2_xc = (obj2.x_min+obj2.x_max) * 0.5; //x-coordinate of center of object 2
				double obj2_yc = (obj2.y_min+obj2.y_max) * 0.5; //y-coordinate of center of object 2
				double obj2_xw = obj2.x_max - obj2.x_min;  //width of object 2
				double obj2_yh = obj2.y_max - obj2.y_min;  //height of object 2

				double len1;  //horizontal difference between centers
				double len2; //vertical difference between centers
				//find the difference between the x and y centers of the two objects
				if(obj1_xc > obj2_xc){
					len1 = obj1_xc - obj2_xc;
				}
...
//REPLACED WITH BUTTONS ON BOARD IN MAIN IMPLEMENTATION BECAUSE KEYPAD STOPPED WORKING
        // returns: 1 - 1
        //              2-2
        //              3-3
        //              0-ignore
